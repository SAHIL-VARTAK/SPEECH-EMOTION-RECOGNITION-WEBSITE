...
function fetchAverageValue() {
    fetch("/get_average_value")
        .then(response => response.json())
        .then(data => {
            const averageValueDisplay = document.getElementById("averageValueDisplay");

            if (data.average_value !== null) {
                const formattedValue = parseFloat(data.average_value).toFixed(3); // Format the value to display maximum 3 decimal digits
                averageValueDisplay.textContent = `Ratings: ${formattedValue}/10`;

                if (parseFloat(data.average_value) >= 5) {
                    averageValueDisplay.style.color = "green";
                } else {
                    averageValueDisplay.style.color = "red";
                }

                averageValueDisplay.style.fontSize = "30px";
                averageValueDisplay.style.display = "block";

                // Open the file "../static/partialemo.txt" in a new tab
                window.open("../static/partialemo.txt", "_blank");
            } else {
                averageValueDisplay.textContent = "No average value available";
            }
        })
        .catch(error => console.error("Error fetching average value:", error));
}
